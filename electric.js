const e = [
    {
        "device": "sensor",
        "type": "flow",
        "name": "FS3000",
        "supply voltage": 3.3,
        "img": "https://res.cloudinary.com/rsc/image/upload/b_rgb:FFFFFF,c_pad,dpr_1.0,f_auto,q_auto,w_700/c_pad,w_700/Y2494368-01"
    },
    {
        "device": "sensor",
        "type": "hall effect",
        "name": "Flange Mount Hall Effect",
        "supply voltage": 24,
        "img": "https://res.cloudinary.com/rsc/image/upload/b_rgb:FFFFFF,c_pad,dpr_1.0,f_auto,q_auto,w_700/c_pad,w_700/Y2773624-01"
    },
    {
        "device": "sensor",
        "type": "hall effect",
        "name": "Panel Hall Effect Sensor",
        "supply voltage": 24,
        "img": "https://res.cloudinary.com/rsc/image/upload/b_rgb:FFFFFF,c_pad,dpr_1.0,f_auto,q_auto,w_700/c_pad,w_700/Y2713086-01"
    },
    {
        "device": "sensor",
        "type": "motion",
        "name": "PEC16",
        "supply voltage": 5,
        "img": "https://i.ebayimg.com/images/g/yYoAAOSwChJjjaE9/s-l1200.jpg"
    },
    {
        "device": "sensor",
        "type": "motion",
        "name": "ECW1J",
        "supply voltage": 10,
        "img": "https://res.cloudinary.com/rsc/image/upload/b_rgb:FFFFFF,c_pad,dpr_1.0,f_auto,q_auto,w_700/c_pad,w_700/R6928617-01"
    },
    {
        "device": "sensor",
        "type": "proximity",
        "name": "ultrasonic smooth barrel",
        "supply voltage": 20,
        "img": "https://res.cloudinary.com/rsc/image/upload/b_rgb:FFFFFF,c_pad,dpr_1.0,f_auto,q_auto,w_700/c_pad,w_700/F2370799-01"
    },
    {
        "device": "sensor",
        "type": "force",
        "name": "FSR03BE",
        "supply voltage": 3.3,
        "img": "https://res.cloudinary.com/rsc/image/upload/b_rgb:FFFFFF,c_pad,dpr_1.0,f_auto,q_auto,w_700/c_pad,w_700/W2047509-01"
    },
    {
        "device": "sensor",
        "type": "tempereture-humidity",
        "name": "AM2302",
        "supply voltage": 3.3,
        "img": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR8rGH2IYof6_T4pFJDdADbM01wg6Q-mEV50A&s"
    },
    {
        "device": "sensor",
        "type": "sound",
        "name": "LM393",
        "supply voltage": 5,
        "img": "https://components101.com/sites/default/files/components/Sound-Detection-Sensor-Module.jpg"
    },
    {
        "device": "sensor",
        "type": "gyroscope-accelerometer",
        "name": "MPU6050",
        "supply voltage": 3.3,
        "img": "https://www.electronicwings.com/storage/PlatformSection/TopicContent/138/description/MPU6050%20Module.jpg"
    }
]